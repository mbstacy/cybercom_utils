...
if (typeof user === "undefined") {
  throw "You have to supply a 'user' variable, à la --eval 'var user = \"admin\"'.";
}
if (typeof password === "undefined") {
  throw "You have to supply a 'password' variable, à la --eval 'var password = \"*********\"'.";
}

//Authenticate Command line must connect to admin database. User must be admin privileges
db.auth(user,password);

if (typeof database === "undefined") {
  throw "You have to supply a 'database' variable, à la --eval 'var database = \"animals\"'.";
}
...
var resultsCollectionName = dbName + "." + collection; // + ".Keys";
//var resultsDB = db.getMongo().getDB("mongoSchema");

db[collection].mapReduce(map, reduce, {
                                  out: { 
                                    replace : resultsCollectionName, 
                                    db :"mongoSchema"},// "varietyResults"},
                                  limit : limit, 
                                  sort : {_id: -1},
                                  scope : { limit : limit }});

var resultsDB = db.getMongo().getDB("mongoSchema"); //"varietyResults");
...
resultsDB[resultsCollectionName].find({}).forEach(function(key) {
 ...
 //   db[resultsCollectionName].save(key);
